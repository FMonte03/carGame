...
let player = { step: 3 };
let keys = { w: false, s: false, d: false, a: false, Space: false };
let score = 0;
...
function moveLines() {
    ...
    lines.forEach(item => {
      ...
      if ( item.getBoundingClientRect().bottom< roadArea.getBoundingClientRect().top){
          item.remove()
...
function startgame() {
    ...
    startBtn.innerHTML = "Use WASD to Navigate";
    init();
}
